...
PictureRenderer.prototype.render = function(sprite) {
    ...
    this._renderNormal(sprite, sprite.getShader(this.renderer.gl) );
};
...
PictureRenderer.prototype._renderNormal = function(sprite, shader) {
    ...
    if (sprite.shader)
    var uniformData = Object.create(sprite.shader.uniformData);
    var textureCount = 1;
    for (var i in uniformData) {
        if (uniformData[i].baseTexture) {
            uniformData[i] = renderer.bindTexture(uniformData[i].baseTexture, textureCount);
            textureCount++;
        }

        shader.uniforms[i] = uniformData[i];
    }
...
PIXI.Sprite.prototype.getShader = function(gl) {
    if (!this.shader) {
        return new PIXI.Shader(gl, 'void main(void){}', 'void main(void){}');
    }

    this._shader = this._shader || new PIXI.Shader(gl, this.shader.vertexSrc, this.shader.fragmentSrc);
